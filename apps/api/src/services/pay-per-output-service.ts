...
export interface BillingStore {
  ...
  heartbeat(uid: string, clientRequestId: string): Promise<void>;
  deduct(
...
export class PayPerOutputService {
  ...
  async execute(input: GenerateOutputInput): Promise<GenerateOutputResult> {
    ...
    try {
      ...
      const heartbeatMs = Math.max(1_000, Math.min(5_000, Math.floor(input.timeoutMs / 3)));
      heartbeatTimer = setInterval(() => {
        void this.billingStore.heartbeat(input.uid, input.clientRequestId);
      }, heartbeatMs);
