...
const serverEnvSchema = z.object({
  ...
  GEMINI_ROUTER_MODEL: z.string().trim().min(1).default('gemini-2.5-pro'),
  ...
  CLAUDE_90_10_AUTOSTART_WORKER: z.coerce.boolean().default(true),
  CLAUDE_90_10_S3_BUCKET: z.string().trim().min(1).optional(),
  CLAUDE_90_10_S3_PREFIX: z.string().trim().min(1).optional(),
  CLAUDE_90_10_S3_ENDPOINT: z.string().trim().url().optional(),
...
export type Claude9010Env = z.infer<typeof claude9010EnvSchema>;
...
export function getClaude9010Env(
  ...
  if (!parsed.success) {
    throw new Error(
      `[claude-90-10-env] ${parsed.error.issues
        .map((issue) => `${issue.path.join('.') || 'root'}: ${issue.message}`)
        .join('; ')}`
    );
  }
